...
fontC = ImageFont.truetype(dir_path+"/hyperlpr_py3/Font/platech.ttf", 30, 0)  # 加载中文字体，38表示字体大小，0表示unicode编码
...
classNames = ('background',
              'plate')
net = dnn.readNetFromCaffe(dir_path+"/hyperlpr_py3/model/MobileNetSSD_test.prototxt",dir_path+"/hyperlpr_py3/model/lpr.caffemodel")  # 读入模型文件
...
#     parser.add_argument('--sdir', default="/home/oem/PycharmProjects/deep_sort_yolov4/hyperlpr_py3/test-imgs",
#                         help='图片输入路径.')
#     parser.add_argument('--rdir', default="/home/oem/PycharmProjects/deep_sort_yolov4/hyperlpr_py3/test-results",
#                         help='识别结果输出路径.')
#     parser.add_argument('--mode', default="3", help='设置识别模式，1平衡 2速度优先 3精度优先.')
...
def lprCar(mode="3"):
    # image = cv2.imdecode(np.fromfile(car_img, dtype=np.uint8), -1)  # 读入图片文件，支持中文名
    image = cv2.imread(filename="/home/oem/PycharmProjects/deep_sort_yolov4/hyperlpr_py3/test-imgs/image-20200808235119208.png")
    # 固定高度，等比例缩放原图片
    h = 1024  # 720 ,image.shape[0] ，指定缩放高度
    scale = image.shape[1] / float(image.shape[0])  # 原图宽高比
    if scale > 1:  # 原图片宽大于高
